...
import type { WPMagazine, WPPost } from "@/types";
...
export default function LibraryClient({ magazines = [], posts = [] }: LibraryClientProps) {
  const { isAuthenticated, openAuthModal } = useAuth();
  const { user, hasAccess } = useAccess();
  const [activeTab, setActiveTab] = useState<LibraryTab>("official");

  const purchasedMagazines = magazines.filter((m) => hasAccess(m.id));
  const fanArticles = posts;
          ...
          {activeTab === "official" ? (
            ...
            >
              ...
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {purchasedMagazines.length === 0 ? (
                  <div className="col-span-full rounded-2xl bg-white p-12 text-center shadow-[var(--shadow-card)]">
                    <BookOpen size={40} className="mx-auto mb-4 text-muted" />
                    <h3 className="font-heading text-lg font-bold text-heading">No Purchased Magazines Found</h3>
                    <p className="mt-2 text-sm text-muted">You haven&apos;t purchased any e-magazines yet. Explore our digital catalog to get started!</p>
                    <Link
                      href="/#magazine"
              ...
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {fanArticles.length === 0 ? (
                  <div className="col-span-full rounded-2xl bg-white p-12 text-center shadow-[var(--shadow-card)]">
                    ...
                    <h3 className="font-heading text-lg font-bold text-heading">No Fan-Writer Articles Yet</h3>
                    <p className="mt-2 text-sm text-muted">Check back later or submit your own article from the dashboard!</p>
                  </div>
                ) : (
